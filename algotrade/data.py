...
from algotrade.upstox_client import fetch_historical_candles
...
def _cache_path(symbol: str, exchange: str, interval: str, start: date, end: date) -> Path:
    name = f"{exchange}_{symbol}_{interval}_{start.isoformat()}_{end.isoformat()}.csv"
...
def get_historical_data(
    ...
    repeated backtests over the same range don't re-hit the Upstox API.
    """
    cache_path = _cache_path(symbol, exchange, interval, start, end)
    ...
    candles = fetch_historical_candles(instrument_key, interval, start, end, config=config)
